...
!   irevz=1
! Version 3.2.5
!   Bug fix: condition dtmin< near still reults in particles getting stuck
...
!define a particle
  type sample
    ...
    real:: conc,mass     ! concentration for all time
...
! define a cell
  type cell
    ...
    double precision:: cmass,mass_remove    ! mass in cell, mass lost since last report
...
! boundary  
  type boundary
    ...
    real:: conc,refresh,nptime              ! aqueous phase concentration,refresh rate control
    ...
    double precision:: pmass,massrate       ! particle mass, rate of injection M/T
    double precision:: mass_remove,maw      ! mass removed
...
! noc             -   number of output control packages (options)
  ...
  character (len=80), parameter:: version=' ---------------------- R W H e t   V e r s i o n   4.1 ----------------------'
!
...
! pnumber      - incremental particle number 
  ...
  double precision:: mass,netmass,netmassp(3),netmassm(3)
  double precision:: massbc(nbtype+1)
...
real por(nzone),ret(nzone),pmass,massrate,refresh,nptime
integer nitype(nbtype),iostatus
...
!.source
  if(itype.eq.1)then
    read(inbnd,*,iostat=iostatus)itype,i,j,kbot,ktop,npart,conc,refresh,tbeg,tend,numbnd
    write(iout,2000,err=9999)nbounds,itype,i,j,kbot,ktop,npart,conc,refresh,tbeg,tend,numbnd
    nitype(itype)=nitype(itype)+1
!.flux condition - source that does not absorb after each time step
  elseif(itype.eq.2)then
    read(inbnd,*,err=9999)itype,x,y,z,sx,sy,sz,pmass,massrate,tbeg,tend,numbnd
    write(iout,2001,err=9999)nbounds,itype,x,y,z,sx,sy,sz,pmass,massrate,tbeg,tend,numbnd
    nitype(itype)=nitype(itype)+1
!.absorbing 
...
!.flux over specified region
  elseif(itype.eq.6)then
    read(inbnd,*,iostat=iostatus)itype,i,j,kbot,ktop,pmass,massrate,tbeg,tend,numbnd
    write(iout,2006,err=9999)nbounds,itype,i,j,kbot,ktop,pmass,massrate,tbeg,tend,numbnd
    nitype(itype)=nitype(itype)+1
!.well with unknown flux, flux calculated from divergence 
  elseif(itype.eq.7)then
    ...
    nitype(itype)=nitype(itype)+1
!.recharge boundary (must have recharge in the cell-bycell flux file)
  elseif(itype.eq.8)then
    read(inbnd,*,err=9999)itype,x,y,sx,sy,pmass,massrate,tbeg,tend,numbnd
    write(iout,2008,err=9999)nbounds,itype,x,y,sx,sy,pmass,massrate,tbeg,tend,numbnd
    nitype(itype)=nitype(itype)+1
!.recharge boundary w/ concentration (must have recharge in the cell-bycell flux file)
  elseif(itype.eq.9)then
    ...
    write(iout,2009,err=9999)nbounds,itype,im,jm,ip,jp,conc,nptime,tbeg,tend,numbnd
    nitype(itype)=nitype(itype)+1
  elseif(itype.eq.10)then
    read(inbnd,*,err=9999)itype,im,jm,km,ip,jp,kp,conc,nptime,tbeg,tend,numbnd
    write(iout,2010,err=9999)nbounds,itype,im,jm,km,ip,jp,kp,conc,nptime,tbeg,tend,numbnd
    nitype(itype)=nitype(itype)+1
  elseif(itype.eq.11)then
    read(inbnd,*,err=9999)itype,im,jm,km,ip,jp,kp,conc,nptime,tbeg,tend,numbnd
    write(iout,2011,err=9999)nbounds,itype,im,jm,km,ip,jp,kp,conc,nptime,tbeg,tend,numbnd
    nitype(itype)=nitype(itype)+1
  endif
!
...
!...location
    if(itype.eq.1)then
      ...
      if(npart.ne.0)then
        bounds(nbounds)%pmass=(ktop-kbot+1)*conc*vol*por(cat(i,j,kbot)%zone)*ret(cat(i,j,kbot)%zone)/npart
      else
        bounds(nbounds)%pmass=0.0
      endif
    ...
    elseif(itype.eq.2)then
      ...
      bounds(nbounds)%massrate=massrate
      bounds(nbounds)%pmass=pmass
    elseif(itype.eq.3)then
      bounds(nbounds)%ijk(1)=i
      bounds(nbounds)%ijk(2)=j
      bounds(nbounds)%kbot=kbot
...
!...zero counters
    ...
    bounds(nbounds)%mass_remove=0
    bounds(nbounds)%maw=0
!...particle masses
!...particle masses
...
do ibtype=1,nbtype+1
  ...
  massbc(itype)=0
enddo
!.....check for multiple boudaries defined at same time and cell location
...
real:: vel3(3,0:nx,0:ny,0:nz),vx4,vy4,vz4,vxyz,v4,u,ux,uy,uz,x,y,z,c,flow
real:: por(nzone),ret(nzone),dlong(nzone),dtran(nzone),ddiff(nzone),decay(nzone)
real, allocatable:: probc(:),probw(:)
integer, allocatable:: loc(:,:)
double precision pmass,massrate,dtsource,trelease,tinitsrc,tnextvelo,pmass2
character (len=80) outfname(nopc)
!.external subroutine to move particles
external movep
  ...
  do isource=1,nsource
    ...
    if(itype.eq.2)then
      ...
      do; if(trelease.gt.sngldt)exit
        ...
        iptype=0
        if(bounds(ibounds)%massrate.gt.0.)call placeu(srctime,xm,ym,zm,sx,sy,sz,pat,cat,1,pmass,iptype,ierr)
        if(bounds(ibounds)%massrate.lt.0.)call placeuf(srctime,xm,ym,zm,sx,sy,sz,pat,cat,vel3,1,pmass,iptype)
...
! Place particles and move
    ...
    elseif(itype.eq.8)then
      ...
      do; if(trelease.gt.sngldt)exit
        ...
        dtsrc=sngl(dt-trelease)
        iptype=0
        if(bounds(ibounds)%massrate.gt.0.)&
...
! cycle through all cells
      do k=km,kp;do j=jm,jp;do i=im,ip
        ...
        do; if(trelease.gt.sngldt)exit
          ...
          enddo
! particle was on edge or outside of domain; igmore and start over 
          mass=mass+pmass
...
real:: vel3(3,0:nx,0:ny,0:nz),c,flow
real:: por(nzone),ret(nzone),dlong(nzone),dtran(nzone),ddiff(nzone),decay(nzone)
real, allocatable:: probc(:),probw(:)
double precision pmass,massrate,dtsource,trelease,tinitsrc
!
...
! o - particle location.
! X - Region over which bilinear interpolation occurs fo this particle.
...
!
  diffxyz=0.0
!.find max representative dispersion coeffient
...
!...compute representative dispersion coeffient 
...
! ***** here is where it goes negative ****
    ...
    if (x > xp) then
        ! print *, 'single xp, x = ', sngl(xp), sngl(x)
...
type (boundary):: bounds(1:maxbnd)
integer source(maxsource)
double precision pmass,tbeg,tend
!
nsource=0
tnextbnd=dble(large)
do ibounds=1,nbounds
...
!.........distribute constant source one last time before turning off
          ...
          massbc(itype)=massbc(itype)+pmass*npart
          xm=bounds(ibounds)%bc_xyzm(1)
          ym=bounds(ibounds)%bc_xyzm(2)
          zm=bounds(ibounds)%bc_xyzm(3)
          sx=bounds(ibounds)%bc_xyzs(1)
...
!.determine extent of patch source
  if(itype.eq.1)then
!...update all constsnt conc. sources at minimum courant condition
...
! make into cummulative histogram
do iw=2,nw
...
do ibtype=1,nbtype+1
  ...
  masstotal=masstotal+massbc(ibtype)
enddo
do idir=1,3
  nptotal=nptotal+netxyzm(idir)
  nptotal=nptotal+netxyzp(idir)
  masstotal=masstotal+netmassm(idir)
  masstotal=masstotal+netmassp(idir)
enddo
...
(sngl(massbc(ibtype)),ibtype=1,nbtype+1),&
(sngl(netmassm(idir)),sngl(netmassp(idir)),idir=1,3),&
sngl(masstotal),sngl(mass),     &

(opcnow(iopc),iopc=1,nopc),abs(sngl(tnextvel)),sngl(tnextbnd),&
sngl(tnextpnt),sngl(tnextopc),sngl(tnextsrc)

if(nran.ne.0)write(iout,1001)sngl(rm)/float(nran)
...
!     &sngl(cor(6)/dfloat(nran/3))
 ...
 1001 format('         Random Number Statistics'/&
       '         AVG          ',5('.'),1x,f15.12/)
...
!...if there is only one boundary cell in this group write its ijk location
    ...
    if(inumbnd-1.eq.1)then
      ...
      if(opc(6).eq.2)write(iunit,'(f15.4,2(",",i8),4(",",e16.10),4(",",i8))')&
...
if(irevz.eq.1)then
  do k=nz,1,-1
    ...
    write(iunit) ((sngl(cat(i,j,k)%cmass/dble(vol*por(cat(i,j,k)%zone)*ret(cat(i,j,k)%zone))),i=1,nx),j=ny,1,-1)
  enddo
else
  do k=1,nz
    write(iunit) ntrans,kstp,kper,sngl(curtime),text,nx,ny,k
    write(iunit) ((sngl(cat(i,j,k)%cmass/dble(vol*por(cat(i,j,k)%zone)*ret(cat(i,j,k)%zone))),i=1,nx),j=1,ny)
  enddo
endif
return
end
...
if(opc(2).eq.2)allocate (cell2(1:ncl), STAT = ierror)
if(ierror.ne.0)stop ' error allocating memory in plotc'
n=1
do icl=1,ncl
  ijk=ncellt(icl)
...
double precision mass_last,netmassm_last(3),netmassp_last(3) 
data netxyzm_last,netxyzp_last,np_last/0,0,0,0,0,0,0/
data netmassm_last,netmassp_last,mass_last/0.,0.,0.,0.,0.,0.,0./
save
!
...
write(iunit,*) np,sngl(curtime)
do ip=1,np	
  write(iunit,'(1x,i15,1x,4(E15.8,1x),D15.8)')&
  pat(1,ip)%pnumber,&  
!  pat(1,ip)%birth_place(1),pat(1,ip)%birth_place(2),pat(1,ip)%birth_place(3),&
  pat(1,ip)%xyz(1),pat(1,ip)%xyz(2),pat(1,ip)%xyz(3),&
...
real:: zdiff,thick,dxy,cmax,msamplet1,msamplet2,msamplet3,msamplet4,msamplet5,mtotal
...
itime=itime+1
if(.not.allocated(msample))allocate (msample(nsam))
msamplet1=0
msamplet3=0
do isam=1,nsam
  sam(isam)%conc=0.0
      ...
      if(dxy.le.sam(isam)%radius.and.zdiff.le.thick)then
        ...
        do izone=1,sam(isam)%nzone
          if(cat(i,j,k)%zone.eq.sam(isam)%zone(izone))then
	        ...
	        msample(isam)=msample(isam)+sngl(pat(1,ip)%pmass)
...
!.....add a particle at location x,y,x,in cell, with mass pmass
use global
implicit none
type (particle):: pat(1,maxnp)
type (cell)::     cat(nx,ny,nz)
...
!
allocate (mcell(nx,ny,nz),pcell(nx,ny,nz))
mcell=0.
pcell=0
do ip=1,np
 i=pat(1,ip)%ijk(1); j=pat(1,ip)%ijk(2); k=pat(1,ip)%ijk(3)
 mcell(i,j,k)=mcell(i,j,k)+pat(1,ip)%pmass
 pcell(i,j,k)=pcell(i,j,k)+1
enddo
do i=1,nx; do j=1,ny; do k=1,nz
  if(abs(cat(i,j,k)%cmass-mcell(i,j,k)).gt.small)then
    write(ibugout,*)' Non-zero Cell Mass-Balance Error '
    write(ibugout,*)' time,i,j,k,tracked mass,current mass',&
    sngl(curtime),i,j,k,cat(i,j,k)%cmass,mcell(i,j,k)
  endif
...
!.........log particle removal in output file
    write(ioutp)pat(1,ip)%pnumber !,'(1x,i15,1x,8(G15.5,1x),G15.5,3(1x,i6))')&
...
double precision:: pmass !,xyz(3),xyzd(3)
...
type (cell)::     cat(nx,ny,nz)
type (boundary):: bounds(1:maxbnd)
! tag particles in absorbing boundaries fo removal
